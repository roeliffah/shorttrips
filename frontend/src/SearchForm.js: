...
function SearchForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Eerst destination_id ophalen
      const resId = await fetch(
        `/bridge/api.php?action=destination-id&city=${encodeURIComponent(city)}&key=${process.env.REACT_APP_API_KEY || "hlIGzfFEk5Af0dWNZO4p"}`
      );
      const dataId = await resId.json();
      if (!dataId.destination_id) {
        setError(`Geen destination_id gevonden voor "${city}".`);
        setLoading(false);
        return;
      }

      // Daarna hotels zoeken
      const params = new URLSearchParams({
        ...
        key: process.env.REACT_APP_API_KEY || "hlIGzfFEk5Af0dWNZO4p",
      });
