...
if ($action === 'quicksearch') {
    ...
    // Landen: parent_id IS NULL
    $stmt = $pdo->query("SELECT id, name FROM bravo_locations WHERE parent_id IS NULL ORDER BY name");
    $countries = $stmt->fetchAll();
    echo json_encode(['results' => $countries]);
    exit;
} elseif ($action === 'regions' && isset($_GET['country_id'])) {
    // Regio's: parent_id = country_id
    $country_id = $_GET['country_id'];
    $stmt = $pdo->prepare("SELECT id, name FROM bravo_locations WHERE parent_id = ? ORDER BY name");
    $stmt->execute([$country_id]);
    $regions = $stmt->fetchAll();
    echo json_encode(['results' => $regions]);
    exit;
} elseif ($action === 'cities' && isset($_GET['region_id'])) {
    // Steden: parent_id = region_id
    ...
    $city = $_GET['city'];
    $stmt = $pdo->prepare("SELECT destination_id FROM bravo_destinations WHERE destination_name = :city");
